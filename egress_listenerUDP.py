...
class ThreadedudpRequestHandler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        ...
        print("[*] Connected from %s on port: %d/udp (client reported %s)" % (self.client_address[0], port, msg))
        if shell == "shell":
            while running:
...
if __name__ == "__main__":
    ...
    try:
        # threaded server to handle multiple udp connections
        socketserver = ThreadedudpServer(('', 0), ThreadedudpRequestHandler)
        ...
        print("[*] Inserting iptables rule to redirect connections from %s to **all udp ports** to Egress Buster port %s/udp" % (listening, port))
        ret = subprocess.Popen(
            "iptables -t nat -A PREROUTING -s %s -i %s -p udp -m udp --dport 1:65535 -j DNAT --to-destination %s:%s" % (srcipaddr, eth, ipaddr, port),
            shell=True
        ).wait()
        if ret != 0:
            raise Exception('failed to set iptables rule (code %d), aborting' % ret)
        print("[*] Listening on all udp ports now... Press control-c when finished.")
        ...
        subprocess.Popen(
            "iptables -t nat -D PREROUTING -s %s -i %s -p udp  --dport 1:65535 -j DNAT --to-destination %s:%s" % (srcipaddr, eth, ipaddr, port),
            shell=True
        ).wait()
    print("[*] Done")
    sys.exit()
